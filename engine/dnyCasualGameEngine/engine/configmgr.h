...
#define CM_MAX_BUFFER_SIZE 2048
#define CM_CVAR_MAX_STRING_LEN 512
#define CM_INVALID_LIST_ID std::wstring::npos
...
namespace ConfigMgr {
	...
	public: 
		...
		struct cvar_s {
			...
			union {
				...
				float fValue;
				double dblValue;
				wchar_t szValue[CM_CVAR_MAX_STRING_LEN];
			};

			virtual std::wstring GetName(void) { return this->szName; }
			...
			virtual std::wstring GetString(void) { return this->szValue; }
			...
			virtual void SetString(std::wstring szValue) { wcscpy_s(this->szValue, szValue.c_str()); }
		};
	private:
		...
		size_t GetCVarId(const std::wstring& szName)
		{
			//Get CVar ID by pointer
	...
	public:
		...
		cvar_s* Add(const std::wstring& szName, const cvar_type_e eType, const std::wstring& szInitialValue)
		{
			//Add CVar
			...
			if (szInitialValue.length()) {
				switch (eType) {
				case CVAR_TYPE_BOOL:
					pCVar->bValue = _wtoi(szInitialValue.c_str()) != 0;
					break;
				case CVAR_TYPE_INT:
		...
		cvar_s* Find(const std::wstring& szName)
		{
			//Find CVar in list
		...
		bool Set(const std::wstring& szName, const bool bValue)
		{
			//Set value
		...
		bool Set(const std::wstring& szName, const int iValue)
		{
			//Set value
		...
		bool Set(const std::wstring& szName, const float fValue)
		{
			//Set value
		...
		bool Set(const std::wstring& szName, const double dblValue)
		{
			//Set value
		...
		bool Set(const std::wstring& szName, const std::wstring& szValue)
		{
			//Set value
		...
		bool SetCast(const std::wstring& szName, const std::wstring& szValue)
		{
			//Set casted value
			...
			switch (pCVar->eType) {
				case CVAR_TYPE_BOOL:
					pCVar->bValue = _wtoi(szValue.c_str()) != 0;
					break;
				case CVAR_TYPE_INT:
		...
		bool Delete(const std::wstring& szName)
		{
	...
	protected:
		bool Find(const std::wstring& szName, size_t* pEntryId)
		{
	...
	public:
		...
		bool Add(const std::wstring& szName, const std::wstring& szDescription, const TCommandCallback pRoutine)
		{
			//Add new command
		...
		bool Handle(const std::wstring& szName)
		{
	...
	private:
		...
		int HandleArg(const std::wstring& szName, int iArgStart)
		{
			...
			for (int i = iArgStart; i < this->m_iCount; i++) { //Loop through args
				...
				pArg->vArgs.push_back(std::wstring(this->m_ppArgs[i]));
	...
	public:
		...
		CCmdLine(wchar_t** ppArgs, const int iArgCount) : m_ppArgs(ppArgs), m_iCount(iArgCount) { this->Collect(); }
		~CCmdLine() { this->Release(); }

		void SetCmdArgs(wchar_t** ppArgs, const int iArgCount) { this->m_ppArgs = ppArgs; this->m_iCount = iArgCount; }
		...
		bool Collect(void)
		{
			...
			for (int i = 1; i < this->m_iCount; i++) { //Loop through args
				...
				if (this->m_ppArgs[i][0] == C_CVAR_CHAR) { //Check for cvar char
					...
					const wchar_t* pszCVarValue = this->m_ppArgs[i + 1];
					if (!pszCVarValue)
						return false;

					//Set cvar value
					this->SetCast(std::wstring(&this->m_ppArgs[i][1]), std::wstring(pszCVarValue));
					...
					int iIgnoreCount = HandleArg(std::wstring(&this->m_ppArgs[i][1]), i + 1);
					if (iIgnoreCount == -1)
						return false;
	...
	typedef void(*TUnknownExpression)(const std::wstring& szExpression);

	class CScriptParser : public CCmdLine, public CCommand {
	private:
		struct script_var_s {
			std::wstring szName;
			std::wstring szReplacer;
		};
		...
		std::vector<std::wstring> m_vExpressionItems; //Items for current handled expression
		...
		size_t FindScriptVariable(const std::wstring& szName)
		{
	...
	public:
		...
		CScriptParser(const TUnknownExpression pfnFunction, const std::wstring& szExpression) : m_pfnUnknownExpressionInform(pfnFunction)
		{
		...
		void SetScriptVariable(const std::wstring& szName, const std::wstring& szValue)
		{
			//Set script variable

			size_t uiScriptVar = this->FindScriptVariable(szName);
			if (uiScriptVar != std::wstring::npos) {
		...
		std::wstring ReplaceVariables(const std::wstring& szExpression)
		{
			//Replace variable values

			std::wstring szResult = szExpression;

			for (size_t i = 0; i < this->m_vScriptVars.size(); i++) {
				size_t uiExprPos = szResult.find(L"%" + this->m_vScriptVars[i].szName + L"%");
				while (uiExprPos != std::wstring::npos) {
					...
					uiExprPos = szResult.find(L"%" + this->m_vScriptVars[i].szName + L"%");
				}
			}
		...
		bool Parse(const std::wstring szExpression)
		{
			//Parse expression
			...
			std::wstring szTemp(L""); //Temporary string
		...
		std::wstring ExpressionItemValue(const size_t uiId)
		{
	...
	private:
		std::wstring m_szScriptDir; //Full path of script directory
	public:
		CConfigInt() {}
		CConfigInt(const std::wstring& szScriptDir) : m_szScriptDir(szScriptDir) {}
		CConfigInt(const std::wstring& szScriptDir, const std::wstring szScript) : m_szScriptDir(szScriptDir)
		{
		...
		void SetScriptDirectory(const std::wstring& szScriptDir)
		{
		...
		bool Execute(const std::wstring& szScriptFile)
		{
			//Execute script file

			std::wifstream hFile(this->m_szScriptDir + szScriptFile); //Open file for reading
			if (hFile.is_open()) { //If file could be opened
				wchar_t szCurrentLine[CM_MAX_BUFFER_SIZE] = {0};

				while (!hFile.eof()) { //While not at end of file
					hFile.getline(szCurrentLine, sizeof(szCurrentLine), '\n'); //Read current line
					
					//Ignore empty lines
					if (!wcslen(szCurrentLine))
						continue;
